  ...
  try {
    ...
    if (userLogin) {
      ...
      if (!isMatch) {
        ...
        // console.log(token);
        /// varifing AdminVer
        const adminVer = await jwt.verify(token, process.env.SECRET_KEY);
        // console.log(adminVer);
        res.status(200).json({
      ...
      // console.log(superAdminLogin);
      const isMatch = await bcrypt.compare(password, superAdminLogin.password);
      if (!isMatch) {
        res.status(400).json({ message: "invalid credintials" });
      } else {
        const token = await jwt.sign(
        ...
        const supAdminVer = await jwt.verify(token, process.env.SECRET_KEY);
